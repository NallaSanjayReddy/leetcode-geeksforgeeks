class Solution {
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        for(int col=0; col<cols; col++) {
            dfs(0, col, pac, heights[0][col], heights);                    //top row surrounds pacific ocean; 4: prevHeight=start height
            dfs(rows-1, col, atl, heights[rows-1][col], heights);          //bottom row surrounds atlantic ocean
        }

        //traverse left and right of island
        for(int row=0; row<rows; row++) {
            dfs(row, 0, pac, heights[row][0], heights);              //left=pacific
            dfs(row, cols-1, atl, heights[row][cols-1], heights);    //right=atlantic
        }
